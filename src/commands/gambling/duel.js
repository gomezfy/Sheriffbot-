...
async function startDuelRound(interaction, state) {
  ...
  const collector = interaction.message.createMessageComponentCollector({
    filter: i => [state.challenger.user.id, state.opponent.user.id].includes(i.user.id),
    time: 15000
  });
  ...
  collector.on('collect', async i => {
    ...
    if (i.user.id === state.challenger.user.id) {
      ...
      await i.reply({ content: `You chose ${ACTIONS[action].emoji} ${ACTIONS[action].name}!`, ephemeral: true });
...
async function resolveRound(message, state) {
  const cAction = ACTIONS[state.challenger.choice];
  const oAction = ACTIONS[state.opponent.choice];
  ...
  if (state.challenger.hp <= 0 || state.opponent.hp <= 0 || state.round >= state.maxRounds) {
    ...
    await startDuelRound({ update: async (data) => message.edit(data), message }, state);
  }
}

async function endDuel(message, state) {
  let winner, loser;
