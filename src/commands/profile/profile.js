...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (isOwnProfile) {
      ...
      await interaction.editReply({ files: [card], components: [editButton] });
    } else {
...
async function createProfileCard(user, stats) {
...
async function drawCurrencyBoxWithImage(ctx, x, y, width, height, iconImage, label, value, color) {
  // Background
  ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
...
async function drawCurrencyBox(ctx, x, y, width, height, icon, label, value, color) {
  // Background
  ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
  roundRect(ctx, x, y, width, height, 12);
  ctx.fill();
...
async function wrapTextWithEmojis(ctx, text, x, y, maxWidth, lineHeight) {
  if (!text || text.trim() === '') {
